...
This vignette illustrates the visualization of a neural net using the tools from the classmap package. We will use the nnet package, which allows the training of simple neural networks with a single hidden layer.
...
representations of the data in the hidden layer (H), as well as in the 
final layer before softmax (X). X is the set of G-dimensional points v_i given by (6) in the paper. This X will be used for the farness in the class maps.
...
We now calculate the predictions of this test data, and reconstruct
the representation of the test data in the hidden and final layer.
